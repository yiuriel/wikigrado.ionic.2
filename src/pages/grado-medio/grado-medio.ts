...
@Component({
  selector: 'page-grado-medio',
  templateUrl: 'grado-medio.html',
})
export class GradoMedioPage {
  ...
  goTo(grade) {
    if (grade) {
      let modal = this.modalCtrl.create(GradoMedioPage, {data: {...grade, type: 'fp_medio', index: grade.id}, dimensionData: this.dimensionData});
      modal.onDidDismiss(() => {
        modal = null;
      })
      modal.present();
